...
class _Clusters(BaseMAB):
    ...
    def __init__(self, rng: np.random.RandomState, arms: List[Arm], n_jobs: int, backend: Optional[str],
        ...
        if is_minibatch:
            self.kmeans = MiniBatchKMeans(n_clusters, random_state=RandomGenerator.get_rng_state(rng))
        else:
            self.kmeans = KMeans(n_clusters, random_state=RandomGenerator.get_rng_state(rng))

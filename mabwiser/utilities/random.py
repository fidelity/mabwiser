...
"""
This module provides random number utilities
"""
...
class _BaseRNG(metaclass=abc.ABCMeta):
    ...
    def choice(
        self,
        a: Union[int, Iterable[int]],
        size: Union[int, Tuple[int]] = None,
        p: Iterable[float] = None,
    ):
    ...
    def multivariate_normal(
        self, mean: List[float], covariance: List[List[float]], size=None
    ):
...
class _NumpyRNG(_BaseRNG):
    ...
    def multivariate_normal(
        ...
        return np.squeeze(
            self.rng.multivariate_normal(mean, covariance, size=size, method="cholesky")
        )

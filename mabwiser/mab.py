...
class MAB:
    ...
    def __init__(self, config: MABConfig):
        ...
        if isinstance(learning_policy, LearningPolicy.EpsilonGreedy):
            lp = _EpsilonGreedy(
                self._rng, self.arms, self.n_jobs, self.backend, learning_policy.epsilon
            )
            ...
            lp = _Softmax(
                self._rng, self.arms, self.n_jobs, self.backend, learning_policy.tau
            )
        elif isinstance(learning_policy, LearningPolicy.ThompsonSampling):
            lp = _ThompsonSampling(
                self._rng,
                self.arms,
                self.n_jobs,
                self.backend,
            ...
            lp = _UCB1(
                self._rng, self.arms, self.n_jobs, self.backend, learning_policy.alpha
            )
            ...
            lp = _Linear(
                ...
                self.backend,
                0,
                learning_policy.epsilon,
                learning_policy.l2_lambda,
                "ridge",
            ...
            check_true(
                False, ValueError("Undefined learning policy " + str(learning_policy))
            )
        ...
        if neighborhood_policy:
            ...
            if isinstance(neighborhood_policy, NeighborhoodPolicy.Clusters):
                self._imp = _Clusters(
                    ...
                    self.backend,
                    lp,
                    neighborhood_policy.n_clusters,
                    neighborhood_policy.is_minibatch,
                )
            elif isinstance(neighborhood_policy, NeighborhoodPolicy.LSHNearest):
                self._imp = _LSHNearest(
                    self._rng,
                    self.arms,
                    self.n_jobs,
                    self.backend,
                    lp,
                    neighborhood_policy.n_dimensions,
                    neighborhood_policy.n_tables,
                ...
                self._imp = _KNearest(
                    ...
                    self.backend,
                    lp,
                    neighborhood_policy.k,
                    neighborhood_policy.metric,
                )
                ...
                check_true(
                    False,
                    ValueError("Undefined context policy " + str(neighborhood_policy)),
                )
        else:
            self.is_contextual = isinstance(
                learning_policy,
    ...
    def learning_policy(self):
        ...
        if isinstance(lp, _EpsilonGreedy):
            ...
            if lp.regression == "ridge":
                ...
                check_true(
                    False, ValueError("Undefined regression " + str(lp.regression))
                )
            ...
            raise NotImplementedError(
                "MAB learning_policy property not implemented for this learning policy."
            )
    ...
    def neighborhood_policy(self):
        ...
        if isinstance(self._imp, _Clusters):
            return NeighborhoodPolicy.Clusters(
                self._imp.n_clusters, isinstance(self._imp.kmeans, MiniBatchKMeans)
            )
            ...
            return NeighborhoodPolicy.LSHNearest(
                self._imp.n_dimensions,
                self._imp.n_tables,
                self._imp.no_nhood_prob_of_arm,
            )
        elif isinstance(self._imp, _Radius):
            return NeighborhoodPolicy.Radius(
                self._imp.radius, self._imp.metric, self._imp.no_nhood_prob_of_arm
            )
    ...
    def add_arm(
        ...
        if binarizer:
            check_true(
                isinstance(self._imp, _ThompsonSampling)
                or isinstance(self._imp.lp, _ThompsonSampling),
                ValueError(
            ...
            check_true(
                callable(binarizer),
                TypeError(
                    "Binarizer must be a callable function that returns True/False "
        ...
        if scaler:
            ...
            check_true(
                hasattr(scaler, "mean_") and hasattr(scaler, "var_"),
                TypeError(
                    "Scaler must be fit with calculated mean_ and var_ attributes"
                ),
            )
        check_false(
            arm in self.arms, ValueError("The arm is already in the list of arms.")
        )
    ...
    def predict(
        self,
        contexts: Union[
            None, List[Num], List[List[Num]], np.ndarray, pd.Series, pd.DataFrame
        ] = None,  # Contexts, optional
    ) -> Union[Arm, List[Arm]]:
    ...
    def warm_start(
        ...
        """
        check_true(
            isinstance(arm_to_features, dict),
            TypeError("Arm features are not given as a dictionary."),
        )
        check_true(
            isinstance(distance_quantile, float),
            TypeError("Distance quantile is not given as a float."),
        )
        check_true(
            0 <= distance_quantile <= 1,
            ValueError("Distance quantile is not between 0 and 1."),
        )
        check_true(
            set(self.arms) == set(arm_to_features.keys()),
    ...
    def _validate_mab_args(
        arms, learning_policy, neighborhood_policy, seed, n_jobs, backend
    ):
        """
        ...
        # Arms
        check_true(
            isinstance(arms, list), TypeError("The arms should be provided in a list.")
        )
        ...
        if neighborhood_policy:
            ...
            if isinstance(neighborhood_policy, NeighborhoodPolicy.TreeBandit):
                check_true(
                    neighborhood_policy._is_compatible(learning_policy),
                    ValueError(
                        "Tree-Bandit is not compatible with the learning policy: "
                        + str(learning_policy)
                    ),
                )
        ...
        check_true(
            ...
            TypeError("Number of parallel jobs must be an integer."),
        )
        check_true(n_jobs != 0, ValueError("Number of parallel jobs cannot be zero."))
        if backend is not None:
            check_true(
                isinstance(backend, str),
                TypeError("Parallel backend must be a string."),
            )
    ...
    def _validate_fit_args(self, decisions, rewards, contexts):
        ...
        if (
            isinstance(self.learning_policy, LearningPolicy.ThompsonSampling)
    ...
    def _validate_context_type(contexts):
        ...
        if isinstance(contexts, np.ndarray):
            check_true(
                contexts.ndim == 2,
                TypeError(
                    "The contexts should be given as 2D list, numpy array, pandas series or data frames."
                ),
            )
    ...
    @staticmethod
    def _convert_matrix(
        matrix_like: Optional[np.ndarray, List, pd.DataFrame, pd.Series],
        row: bool = False,
        ...
        if contexts is None:
            ...
            if is_called_from_fit:
                if len(decisions) > 1:  # multiple decisions exists
                    return np.asarray(contexts.values, order="C").reshape(
                        -1, 1
                    )  # go from 1D to 2D
                ...
                if isinstance(
                    ...
                    for arm in self.arms:
                        try:
                            num_features = len(
                                self._imp.arm_to_tree[arm].feature_importances_
                            )
                        except:
                            continue
                else:

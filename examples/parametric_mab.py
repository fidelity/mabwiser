...
from mabwiser.configs.learning import LinUCB, LinTS
from mabwiser.configs.neighborhood import Radius, KNearest
...
linucb = MAB(config=MABConfig(
    arms=ads,
    learning_policy=LinUCB(alpha=1.25, l2_lambda=1.0),
    seed=123456
))
...
linucb.add_arm(ArmConfig(arm="6"))

# Warm start new arm
linucb.warm_start(WarmStartConfig(arm_to_features=arm_to_features, distance_quantile=0.75))
...
radius = MAB(config=MABConfig(
    arms=ads,
    learning_policy=LinUCB(alpha=1.0),
    neighborhood_policy=Radius(radius=1.0),
    seed=123456
))
...
lints = MAB(config=MABConfig(
    arms=ads,
    learning_policy=LinTS(alpha=1.5, l2_lambda=1.0),
))
...
radius = MAB(config=MABConfig(
    arms=ads,
    learning_policy=LinTS(alpha=0.5),
    neighborhood_policy=Radius(radius=1.0)
))
...
knearest = MAB(config=MABConfig(
    arms=ads,
    learning_policy=LinTS(alpha=1.0),
    neighborhood_policy=KNearest(k=4)
))

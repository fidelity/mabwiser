...
from mabwiser.configs.neighborhood import Radius, KNearest, Clusters, LSHNearest, TreeBandit
...
lshnearest = MAB(
    MABConfig(
        arms=ads,
        learning_policy=UCB1(alpha=1.25),
        neighborhood_policy=LSHNearest(n_dimensions=5, n_tables=5)
    )
)
...
# Updating of the model with new arm
lshnearest.add_arm(ArmConfig(arm="6"))
...
treebandit = MAB(
    MABConfig(
        arms=ads,
        learning_policy=UCB1(alpha=1.25),
        neighborhood_policy=TreeBandit()
    )
)
...
# Updating of the model with new arm
treebandit.add_arm(ArmConfig(arm="6"))